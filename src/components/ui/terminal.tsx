...
export function useAudio(enabled: boolean) {
  ...
  const playSound = (sound: [number, number] | undefined) => {
    if (!readyRef.current || !ctxRef.current || !sound) return;
    ...
    if (bufferRef.current) {
      ...
      try {
        ...
        osc.type = "sine";
        osc.frequency.setValueAtTime(450, ctxRef.current.currentTime);
        osc.frequency.exponentialRampToValueAtTime(110, ctxRef.current.currentTime + 0.025);
        gain.gain.setValueAtTime(0.06, ctxRef.current.currentTime);
        gain.gain.exponentialRampToValueAtTime(0.001, ctxRef.current.currentTime + 0.025);
        osc.connect(gain);
        gain.connect(ctxRef.current.destination);
        osc.start();
        osc.stop(ctxRef.current.currentTime + 0.025);
      } catch {}
    }
  };
...
export function Terminal({
  ...
  return (
    <div
      ref={containerRef}
      className={cn(
        "mx-auto w-full max-w-4xl font-mono text-xs md:text-sm",
        className,
      )}
    >
      <div className="overflow-hidden rounded-xl border border-neutral-800 bg-neutral-900 shadow-2xl">
        {/* Title Bar */}
        <div className="flex items-center gap-2 bg-neutral-800/90 px-4 py-3 border-b border-neutral-700/50">
          <div className="flex items-center gap-2">
            <div className="h-3 w-3 rounded-full bg-red-500 transition-colors hover:bg-red-600 shadow-sm" />
            <div className="h-3 w-3 rounded-full bg-yellow-500 transition-colors hover:bg-yellow-600 shadow-sm" />
            <div className="h-3 w-3 rounded-full bg-green-500 transition-colors hover:bg-green-600 shadow-sm" />
          </div>
          <div className="flex-1 text-center">
            <span className="truncate font-mono text-xs font-semibold tracking-wider text-neutral-300">
              {username} — bash — 80×24
            </span>
          </div>
        ...
        <div
          ref={contentRef}
          className="no-visible-scrollbar h-[360px] md:h-[420px] overflow-y-auto p-5 font-mono text-xs md:text-sm"
        >
          {lines.map((line, i) => (
            <div key={i} className="leading-relaxed whitespace-pre-wrap mb-1.5">
              {line.type === "command" ? (
                <span>
                  {prompt}
                ...
                <span className="text-neutral-300 pl-4 block border-l-2 border-emerald-500/30 my-1 py-0.5">
                  {line.content}
                </span>
              )}
            </div>
